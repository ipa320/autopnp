 ...
 *
 * $Id: loginClientManifest.c 7768 2014-03-11 15:08:26Z geisinger $
 */
...
)
{
    ...
    componentManifest->componentType = (xme_core_componentType_t)11;
    componentManifest->componentWrapperInit = xme_core_login_loginClientComponentWrapper_init;
    componentManifest->componentWrapperReceivePort = xme_core_login_loginClientComponentWrapper_receivePort;
    componentManifest->componentWrapperFini = xme_core_login_loginClientComponentWrapper_fini;
    componentManifest->componentInit = (xme_core_componentManifest_componentInit_t)xme_core_login_loginClientComponent_init;
    (void)xme_hal_safeString_strncpy(componentManifest->name, "loginClient", sizeof(componentManifest->name));
    componentManifest->configStructSize = sizeof(xme_core_login_loginClientComponent_config_t);

    {
        uint32_t functionArrayLength = sizeof(componentManifest->functionManifests) / sizeof(componentManifest->functionManifests[0]);

        // Function 'loginClientSendRequest'
        {
            if (0U >= functionArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0U == functionArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'loginClient' defines more functions (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                       2,
                       functionArrayLength
            ...
            else
            {
                ...
                functionManifest->requiredPortIndices[0] = (xme_core_login_loginClientComponentWrapper_internalPortId_t)XME_CORE_LOGIN_LOGINCLIENTCOMPONENTWRAPPER_PORT_OUTREQUEST;
                functionManifest->optionalPortIndicesLength = 0;
                functionManifest->functionInit = (xme_core_exec_initCallback_t)xme_core_login_loginClientSendRequestFunction_init;
                functionManifest->functionFini = (xme_core_exec_finiCallback_t)xme_core_login_loginClientSendRequestFunction_fini;
                functionManifest->functionWrapperExecute = xme_core_login_loginClientSendRequestFunctionWrapper_execute;
            }
        }
        // Function 'loginClientProcessResponse'
        {
            ...
            else
            {
                ...
                functionManifest->completion = true;
                functionManifest->requiredPortIndicesLength = 1;
                functionManifest->requiredPortIndices[0] = (xme_core_login_loginClientComponentWrapper_internalPortId_t)XME_CORE_LOGIN_LOGINCLIENTCOMPONENTWRAPPER_PORT_INRESPONSE;
                functionManifest->optionalPortIndicesLength = 1;
                functionManifest->optionalPortIndices[0] = (xme_core_login_loginClientComponentWrapper_internalPortId_t)XME_CORE_LOGIN_LOGINCLIENTCOMPONENTWRAPPER_PORT_OUTLOGINACKNOWLEDGMENT;
                functionManifest->functionInit = (xme_core_exec_initCallback_t)xme_core_login_loginClientProcessResponseFunction_init;
                functionManifest->functionFini = (xme_core_exec_finiCallback_t)xme_core_login_loginClientProcessResponseFunction_fini;
                functionManifest->functionWrapperExecute = xme_core_login_loginClientProcessResponseFunctionWrapper_execute;
            }
        }
    }

    {
        ...
        // Subscription 'inResponse'
        {
            if (0 >= portArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0 == portArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'loginClient' defines more ports (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                        3,
                        portArrayLength
        ...
        // Publication 'outLoginAcknowledgment'
        {
