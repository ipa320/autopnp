 ...
 *
 * $Id: rOSManifest.cpp 7769 2014-03-11 15:27:13Z geisinger $
 */
...
#include "../include/rOSComponentWrapper.h"
...
)
{
    ...
    componentManifest->componentWrapperInit = rOSGateway_adv_rOS_rOSComponentWrapper_init;
    componentManifest->componentWrapperReceivePort = NULL;
    componentManifest->componentWrapperFini = rOSGateway_adv_rOS_rOSComponentWrapper_fini;
    componentManifest->componentInit = (xme_core_componentManifest_componentInit_t)rOSGateway_adv_rOS_rOSComponent_init;
    (void)xme_hal_safeString_strncpy(componentManifest->name, "ROS", sizeof(componentManifest->name));
    componentManifest->configStructSize = sizeof(rOSGateway_adv_rOS_rOSComponent_config_t);
        ...
        // Function 'executeROS'
        {
            if (0U >= functionArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0U == functionArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'ROS' defines more functions (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                       1,
                       functionArrayLength
            ...
            else
            {
                ...
                functionManifest->functionInit = (xme_core_exec_initCallback_t)rOSGateway_adv_rOS_executeROSFunction_init;
                functionManifest->functionFini = (xme_core_exec_finiCallback_t)rOSGateway_adv_rOS_executeROSFunction_fini;
                functionManifest->functionWrapperExecute = rOSGateway_adv_rOS_executeROSFunctionWrapper_execute;
            }
        }
    }
