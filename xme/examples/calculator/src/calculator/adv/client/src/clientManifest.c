 ...
 *
 * $Id: clientManifest.c 7769 2014-03-11 15:27:13Z geisinger $
 */
...
)
{
    ...
    componentManifest->componentWrapperInit = calculator_adv_client_clientComponentWrapper_init;
    componentManifest->componentWrapperReceivePort = calculator_adv_client_clientComponentWrapper_receivePort;
    componentManifest->componentWrapperFini = calculator_adv_client_clientComponentWrapper_fini;
    componentManifest->componentInit = (xme_core_componentManifest_componentInit_t)calculator_adv_client_clientComponent_init;
    (void)xme_hal_safeString_strncpy(componentManifest->name, "client", sizeof(componentManifest->name));
    componentManifest->configStructSize = sizeof(calculator_adv_client_clientComponent_config_t);

    {
        ...
        // Function 'sendRequest'
        {
            if (0U >= functionArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0U == functionArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'client' defines more functions (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                       2,
                       functionArrayLength
            ...
            else
            {
                ...
                functionManifest->requiredPortIndices[0] = (calculator_adv_client_clientComponentWrapper_internalPortId_t)CALCULATOR_ADV_CLIENT_CLIENTCOMPONENTWRAPPER_PORT_CLIENTREQUEST;
                functionManifest->optionalPortIndicesLength = 0;
                functionManifest->functionInit = (xme_core_exec_initCallback_t)calculator_adv_client_sendRequestFunction_init;
                functionManifest->functionFini = (xme_core_exec_finiCallback_t)calculator_adv_client_sendRequestFunction_fini;
                functionManifest->functionWrapperExecute = calculator_adv_client_sendRequestFunctionWrapper_execute;
            }
        }
        // Function 'printResponse'
        {
            ...
            else
            {
                ...
                functionManifest->completion = true;
                functionManifest->requiredPortIndicesLength = 1;
                functionManifest->requiredPortIndices[0] = (calculator_adv_client_clientComponentWrapper_internalPortId_t)CALCULATOR_ADV_CLIENT_CLIENTCOMPONENTWRAPPER_PORT_CLIENTRESPONSE;
                functionManifest->optionalPortIndicesLength = 0;
                functionManifest->functionInit = (xme_core_exec_initCallback_t)calculator_adv_client_printResponseFunction_init;
                functionManifest->functionFini = (xme_core_exec_finiCallback_t)calculator_adv_client_printResponseFunction_fini;
                functionManifest->functionWrapperExecute = calculator_adv_client_printResponseFunctionWrapper_execute;
            }
        }
    }
        ...
        // Subscription 'clientResponse'
        {
            if (0 >= portArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0 == portArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'client' defines more ports (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                        2,
                        portArrayLength
