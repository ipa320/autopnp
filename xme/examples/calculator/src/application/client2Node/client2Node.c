 ...
 *
 * $Id: client2Node.c 7805 2014-03-13 09:54:35Z geisinger $
 */

/**
 * \file
 *         Main file of node 'client2Node'.
 *
 * \author
...
// PROTECTED REGION ID(CLIENT2NODE_INCLUDES) ENABLED START
...
static xme_core_exec_functionDescriptor_t* client2FuncDescs[2] = {NULL, NULL};
...
)
{
    ...
    // PROTECTED REGION ID(CLIENT2NODE_XME_INIT) ENABLED START
    ...
    xme_core_node_setNodeName("client2Node");
...
{
	//TODO: Remove protected region when issue #3982 has been resolved!
    // PROTECTED REGION ID(CLIENT2NODE_XME_FINI) ENABLED START
...
configureSchedule(void)
{
    ...
    // Component 'client2' function 'sendRequest'
    if (NULL != client2FuncDescs[0])
    {
        status = xme_core_exec_dispatcher_createFunctionExecutionUnit(client2FuncDescs[0], true);
        XME_ASSERT_NORVAL(XME_STATUS_SUCCESS == status);
    }
    // Component 'client2' function 'printResponse'
    if (NULL != client2FuncDescs[1])
    {
        status = xme_core_exec_dispatcher_createFunctionExecutionUnit(client2FuncDescs[1], true);
        XME_ASSERT_NORVAL(XME_STATUS_SUCCESS == status);
    }
    ...
    if (NULL != client2FuncDescs[0])
    {
        // Add function 'sendRequest' of component instance 'client2' of type 'client' to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
            client2FuncDescs[0]->componentId,
            client2FuncDescs[0]->functionId,
            client2FuncDescs[0]->taskArgs,
    ...
    if (NULL != client2FuncDescs[1])
    {
        // Add function 'printResponse' of component instance 'client2' of type 'client' to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
            client2FuncDescs[1]->componentId,
            client2FuncDescs[1]->functionId,
            client2FuncDescs[1]->taskArgs,
...
createComponentInstances(void)
{
    ...
    // Create statically configured component instance 'client2' of type 'client'
    {
        ...
        XME_CHECK_MSG(NULL != builder, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'client2' of type 'client' failed. Builder could not be created.\n", __FILE__, __LINE__);
        ...
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'client2' of type 'client' failed. Building of component failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_setStateToAnnounced(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'client2' of type 'client' failed. Setting state to ANNOUNCED failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_createAndRegisterComponent(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'client2' of type 'client' failed. Registration and port allocation failed.\n", __FILE__, __LINE__);

        ports[0] = getDataPacketID((xme_core_component_t)128, 0);
        ports[1] = getDataPacketID((xme_core_component_t)128, 1);
        
        client2FuncDescs[0] = getFunctionDescriptor((xme_core_component_t)128, 0);
        client2FuncDescs[1] = getFunctionDescriptor((xme_core_component_t)128, 1);
    }
...
main(int argc, char* argv[])
{
    ...
    xme_hal_env_setConsoleTitle("client2Node");
    ...
    // PROTECTED REGION ID(CLIENT2NODE_MAIN_BEFORE_COMPONENT_CREATION) ENABLED START
    ...
    // PROTECTED REGION ID(CLIENT2NODE_MAIN_RUN_BEFORE) ENABLED START
    ...
    // PROTECTED REGION ID(CLIENT2NODE_MAIN_RUN_AFTER) ENABLED START
    ...
    XME_LOG(XME_LOG_NOTE, "Node client2Node (nodeID %d) has shut down.\n", xme_core_node_getCurrentNodeId());
