 ...
 *
 * $Id: serverNode.c 7805 2014-03-13 09:54:35Z geisinger $
 */

/**
 * \file
 *         Main file of node 'serverNode'.
 *
 * \author
...
// PROTECTED REGION ID(SERVERNODE_INCLUDES) ENABLED START
 ...
 *
 * \details Index | Port
 *              0 | calculator.calculatorRequest
 *              1 | calculator.calculatorResponse
 *              2 | demarshaler.configuration[0].out (calculator.calculatorRequest)
 *              3 | demarshaler.configuration[1].out (calculator.calculatorRequest)
 *              4 | demarshaler.configuration[0].in (calculator.calculatorRequest)
 *              5 | demarshaler.configuration[1].in (calculator.calculatorRequest)
 *              6 | udpReceive.configuration[0].out (calculator.calculatorRequest)
 *              7 | udpReceive.configuration[1].out (calculator.calculatorRequest)
 *              8 | channelSelector.configuration[0].out (calculator.calculatorResponse)
 *              9 | channelSelector.configuration[0].out (calculator.calculatorResponse)
 *             10 | channelSelector.configuration[0].in (calculator.calculatorResponse)
 *             11 | marshaler.configuration[0].out (calculator.calculatorResponse)
 *             12 | marshaler.configuration[1].out (calculator.calculatorResponse)
 *             13 | marshaler.configuration[0].in (calculator.calculatorResponse)
 *             14 | marshaler.configuration[1].in (calculator.calculatorResponse)
 *             15 | udpSend.configuration[0].in (calculator.calculatorResponse)
 *             16 | udpSend.configuration[1].in (calculator.calculatorResponse)
 */
static xme_core_dataManager_dataPacketId_t ports[17];
 ...
 */
static void* channelSelectorBuffers[1];
/**
 * \brief Buffers required by UdpSendWaypoint.
 */
static void* udpSendBuffers[2];
static xme_core_exec_functionDescriptor_t* calculatorFuncDescs[1] = {NULL};
...
static xme_wp_waypoint_instanceId_t channelSelectorWaypointInstanceIds[1] = { XME_WP_WAYPOINT_INSTANCEID_INVALID };
static xme_core_exec_functionDescriptor_t* marshalerWaypointDescriptor = NULL;
static xme_wp_waypoint_instanceId_t marshalerWaypointInstanceIds[2] = { XME_WP_WAYPOINT_INSTANCEID_INVALID, XME_WP_WAYPOINT_INSTANCEID_INVALID };
...
)
{
    ...
    // PROTECTED REGION ID(SERVERNODE_XME_INIT) ENABLED START
    ...
    xme_core_node_setNodeName("serverNode");
...
{
	//TODO: Remove protected region when issue #3982 has been resolved!
    // PROTECTED REGION ID(SERVERNODE_XME_FINI) ENABLED START
...
configureSchedule(void)
{
    ...
    // Component 'calculator' function 'calculate'
    if (NULL != calculatorFuncDescs[0])
    {
        status = xme_core_exec_dispatcher_createFunctionExecutionUnit(calculatorFuncDescs[0], true);
    ...
    {
        // Add udpreceive waypoint configuration for component port 'calculator.calculatorRequest'to schedule
    ...
    {
        // Add demarshaler waypoint configuration for component port 'calculator.calculatorRequest'to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
    ...
    if (NULL != calculatorFuncDescs[0])
    {
        // Add function 'calculate' of component instance 'calculator' of type 'calculator' to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
            calculatorFuncDescs[0]->componentId,
            calculatorFuncDescs[0]->functionId,
            calculatorFuncDescs[0]->taskArgs,
    ...
    {
        // Add channelselector waypoint configuration for component port 'calculator.calculatorResponse'to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
    ...
    {
        // Add marshaler waypoint configuration for component port 'calculator.calculatorResponse'to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
    ...
    {
        // Add udpsend waypoint configuration for component port 'calculator.calculatorResponse'to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
...
createComponentInstances(void)
{
    ...
    // Create statically configured component instance 'calculator' of type 'calculator'
    {
        xme_core_nodeMgr_compRep_componentBuilder_t* builder = NULL;
        xme_core_nodeMgr_compRep_componentHandle_t componentHandle;

        builder = xme_core_nodeMgr_compRep_createBuilder(XME_CORE_NODE_LOCAL_NODE_ID, (xme_core_componentType_t)5000);
        XME_CHECK_MSG(NULL != builder, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'calculator' of type 'calculator' failed. Builder could not be created.\n", __FILE__, __LINE__);
        xme_core_nodeMgr_compRep_builderSetComponentID(builder, (xme_core_component_t)128);
        xme_core_nodeMgr_compRep_builderSetQueueSize(builder, 0, 2u); // Port 'calculatorRequest'
        status = xme_core_nodeMgr_compRep_build(builder, &componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'calculator' of type 'calculator' failed. Building of component failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_setStateToAnnounced(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'calculator' of type 'calculator' failed. Setting state to ANNOUNCED failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_createAndRegisterComponent(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'calculator' of type 'calculator' failed. Registration and port allocation failed.\n", __FILE__, __LINE__);

        ports[0] = getDataPacketID((xme_core_component_t)128, 0);
        ports[1] = getDataPacketID((xme_core_component_t)128, 1);
        
        calculatorFuncDescs[0] = getFunctionDescriptor((xme_core_component_t)128, 0);
    }
...
createWaypoints(void)
{
...

    {
        ...
        if (NULL != demarshalerWaypointDescriptor)
        {
            ...
            (
                demarshalerWaypointDescriptor,
                &ports[4], // demarshaler.configuration[0].in (calculator.calculatorRequest)
                1,
                &ports[2], // demarshaler.configuration[0].out (calculator.calculatorRequest)
            ...
            (
                demarshalerWaypointDescriptor,
                &ports[5], // demarshaler.configuration[1].in (calculator.calculatorRequest)
                1,
                &ports[3], // demarshaler.configuration[1].out (calculator.calculatorRequest)
        ...
        {
            {
                ...
                (
                    udpReceiveWaypointDescriptor,
                    &ports[6], // udpReceive.configuration[0].out (calculator.calculatorRequest)
                    key,
                    33221,
                    CALCULATOR_TOPIC_CALCULATORREQUEST,
                    sizeof(calculator_topic_calculatorRequest_t),
            ...
            {
                uint8_t key[] = {0, 0, 39, 17};
        
                udpReceiveWaypointAddConfig
                (
                    udpReceiveWaypointDescriptor,
                    &ports[7], // udpReceive.configuration[1].out (calculator.calculatorRequest)
                    key,
                    33221,
                    CALCULATOR_TOPIC_CALCULATORREQUEST,
                    sizeof(calculator_topic_calculatorRequest_t),
...

    {
        ...
        if (NULL != channelSelectorWaypointDescriptor)
        {
            ...
            (
                ...
                &ports[10], // channelSelector.configuration[0].in (calculator.calculatorResponse)
                1,
                &ports[8], // channelSelector.configuration[0].out (calculator.calculatorResponse)
            ...
            (
                ...
                &ports[10], // channelSelector.configuration[0].in (calculator.calculatorResponse)
                1,
                &ports[9], // channelSelector.configuration[0].out (calculator.calculatorResponse)
                CALCULATOR_TOPIC_CALCULATORRESPONSE,
                sizeof(calculator_topic_calculatorResponse_t),
                (xme_core_channelId_t) 10001,
...

    {
        ...
        if (NULL != marshalerWaypointDescriptor)
        {
            ...
            (
                marshalerWaypointDescriptor,
                &ports[13], // marshaler.configuration[0].in (calculator.calculatorResponse)
                1,
                &ports[11], // marshaler.configuration[0].out (calculator.calculatorResponse)
            ...
            (
                marshalerWaypointDescriptor,
                &ports[14], // marshaler.configuration[1].in (calculator.calculatorResponse)
                1,
                &ports[12], // marshaler.configuration[1].out (calculator.calculatorResponse)
        ...
        {
            {
                ...
                (
                    udpSendWaypointDescriptor,
                    &ports[15], // udpSend.configuration[0].in (calculator.calculatorResponse)
                    key,
                    "127.0.0.1",
                    33222,
                    CALCULATOR_TOPIC_CALCULATORRESPONSE,
            ...
            {
                uint8_t key[] = {0, 0, 39, 19};
        
                udpSendWaypointAddConfig
                (
                    udpSendWaypointDescriptor,
                    &ports[16], // udpSend.configuration[1].in (calculator.calculatorResponse)
                    key,
                    "127.0.0.1",
                    33223,
                    CALCULATOR_TOPIC_CALCULATORRESPONSE,
...
createComponentTypeManifests(void)
{
    ...
    // Create and add manifest for component type 'calculator' to manifest repository
    {
        xme_core_componentManifest_t componentManifest;

        status = calculator_adv_calculator_manifest_createComponentTypeManifest(&componentManifest);

        if (XME_STATUS_SUCCESS != status)
        {
            XME_LOG(XME_LOG_WARNING, "Creation of component type manifest for 'calculator' failed.\n");
        }
        else
        {
            ...
            if (XME_STATUS_SUCCESS != status)
            {
                XME_LOG(XME_LOG_WARNING, "Addtion of component type manifest to manifest repository failed for 'calculator'.\n");
            }
        }
    }
}
...
registerComponentTypesInLoggingEngine(void)
{
    ...
    {
        // Register component type 'calculator' in logging engine
        status = xme_core_log_registerComponent((xme_core_log_componentID_t) 5000, "calculator", xme_core_log_getMinimumLogSeverityLevel());
        XME_LOG_IF
        (
...
addDataPacketTransferEntries(void)
{
    ...
    status = xme_core_broker_addDataPacketTransferEntry(ports[1], ports[10]); // calculator.calculatorResponse -> channelSelector.configuration[0].in (calculator.calculatorResponse)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[1], ports[10], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[2], ports[0]); // demarshaler.configuration[0].out (calculator.calculatorRequest) -> calculator.calculatorRequest
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[2], ports[0], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[3], ports[0]); // demarshaler.configuration[1].out (calculator.calculatorRequest) -> calculator.calculatorRequest
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[3], ports[0], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[6], ports[4]); // udpReceive.configuration[0].out (calculator.calculatorRequest) -> demarshaler.configuration[0].in (calculator.calculatorRequest)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[6], ports[4], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[7], ports[5]); // udpReceive.configuration[1].out (calculator.calculatorRequest) -> demarshaler.configuration[1].in (calculator.calculatorRequest)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[7], ports[5], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[8], ports[13]); // channelSelector.configuration[0].out (calculator.calculatorResponse) -> marshaler.configuration[0].in (calculator.calculatorResponse)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[8], ports[13], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[9], ports[14]); // channelSelector.configuration[0].out (calculator.calculatorResponse) -> marshaler.configuration[1].in (calculator.calculatorResponse)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[9], ports[14], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[11], ports[15]); // marshaler.configuration[0].out (calculator.calculatorResponse) -> udpSend.configuration[0].in (calculator.calculatorResponse)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[11], ports[15], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[12], ports[16]); // marshaler.configuration[1].out (calculator.calculatorResponse) -> udpSend.configuration[1].in (calculator.calculatorResponse)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[12], ports[16], status);
...
main(int argc, char* argv[])
{
    ...
    xme_hal_env_setConsoleTitle("serverNode");
    ...
    // PROTECTED REGION ID(SERVERNODE_MAIN_BEFORE_COMPONENT_CREATION) ENABLED START
    ...
    // PROTECTED REGION ID(SERVERNODE_MAIN_RUN_BEFORE) ENABLED START
    ...
    // PROTECTED REGION ID(SERVERNODE_MAIN_RUN_AFTER) ENABLED START
    ...
    XME_LOG(XME_LOG_NOTE, "Node serverNode (nodeID %d) has shut down.\n", xme_core_node_getCurrentNodeId());
