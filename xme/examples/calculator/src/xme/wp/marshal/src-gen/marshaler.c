 ...
 *
 * $Id: marshaler.c 7769 2014-03-11 15:27:13Z geisinger $
 */
...
/**
 * \brief  Performs marshaling for topic 'calculatorResponse'.
 *
 ...
 */
xme_status_t
doMarshalingForCalculatorResponse
(
    xme_core_dataManager_dataPacketId_t inputPort,
    xme_core_dataManager_dataPacketId_t outputPort,
    void* buffer
...
/**
 * \brief  Performs marshaling for topic 'calculatorRequest'.
 *
 * \param  inputPort Input port where data is stored that will be marshaled.
 * \param  buffer Buffer where marshaled data will be written to.
 *
 ...
 */
xme_status_t
doMarshalingForCalculatorRequest
(
    xme_core_dataManager_dataPacketId_t inputPort,
    xme_core_dataManager_dataPacketId_t outputPort,
    void* buffer
...
)
{
    ...
    if (CALCULATOR_TOPIC_CALCULATORRESPONSE == topic)
    {
        ...
        status = doMarshalingForCalculatorResponse
        (
            inputPort,
            outputPort,
            (void*)marshaledData
    ...
    else if (CALCULATOR_TOPIC_CALCULATORREQUEST == topic)
    {
        uint8_t marshaledData[9];
        
        buffer = marshaledData;
        bufferSize = 9;
        
        status = doMarshalingForCalculatorRequest
        (
            inputPort,
            outputPort,
            (void*)marshaledData
...
{
    calculator_topic_calculatorResponse_t topicData;
    ...
    topicDataSize = sizeof(calculator_topic_calculatorResponse_t);
    ...
    // double topicData.result
    {
        ...
        value.d = topicData.result;
    
        value.i = xme_hal_net_htonll(value.i);
        xme_hal_mem_copy(bufferPtr, &value, 8);
        bufferPtr += 8;
    }
...
{
    calculator_topic_calculatorRequest_t topicData;
    unsigned int topicDataSize;
    unsigned int bytesRead;
    uint8_t* bufferPtr;
    xme_status_t status;
    ...
    topicDataSize = sizeof(calculator_topic_calculatorRequest_t);
    ...
    // int32_t topicData.operand1
    {
        uint32_t netValue;
    
        netValue = xme_hal_net_htonl((uint32_t)topicData.operand1);
        xme_hal_mem_copy(bufferPtr, &netValue, sizeof(uint32_t));
        bufferPtr += sizeof(uint32_t);
    }
    
    // int32_t topicData.operand2
    {
        uint32_t netValue;
    
        netValue = xme_hal_net_htonl((uint32_t)topicData.operand2);
        xme_hal_mem_copy(bufferPtr, &netValue, sizeof(uint32_t));
        bufferPtr += sizeof(uint32_t);
    }
    
    // enum topicData.operation
    {
        uint8_t netValue;
    
        netValue = (uint8_t)topicData.operation;
