 ...
 *
 * $Id: lonelyNode.c 7805 2014-03-13 09:54:35Z geisinger $
 */

/**
 * \file
 *         Main file of node 'lonelyNode'.
 *
 * \author
...
// PROTECTED REGION ID(LONELYNODE_INCLUDES) ENABLED START
 ...
 *
 * \details Index | Port
 *              0 | loginClient.inResponse
 *              1 | loginClient.outRequest
 *              2 | loginClient.outLoginAcknowledgment
 *              3 | monitorB.sensorValueIn
 ...
 *             10 | demarshaler.configuration[0].out (pnpClient.inGraph)
 *             11 | demarshaler.configuration[1].out (loginClient.inResponse)
 *             12 | demarshaler.configuration[0].in (pnpClient.inGraph)
 *             13 | demarshaler.configuration[1].in (loginClient.inResponse)
 *             14 | udpReceive.configuration[0].out (pnpClient.inGraph)
 *             15 | udpReceive.configuration[1].out (loginClient.inResponse)
...
)
{
    ...
    // PROTECTED REGION ID(LONELYNODE_XME_INIT) ENABLED START
    ...
    // PROTECTED REGION END
     ...
     // Add interface to node registry
     {
         ...
         (void)xme_hal_safeString_strncpy(addressBuffer, "127.0.0.1:33225", sizeof(addressBuffer));
    ...
    xme_core_node_setNodeName("lonelyNode");
...
{
	//TODO: Remove protected region when issue #3982 has been resolved!
    // PROTECTED REGION ID(LONELYNODE_XME_FINI) ENABLED START
...
configureSchedule(void)
{
    ...
    if (NULL != demarshalerWaypointDescriptor)
    {
        ...
        (
            ...
            xme_hal_time_timeIntervalFromMilliseconds(70ull),
            xme_hal_time_timeIntervalFromMilliseconds(5ull),
            1,
            0,
            true
...
createComponentInstances(void)
{
    ...
    // Create NON-statically configured component instance 'monitorB' of type 'monitorB'
    {
    ...
    // Create statically configured component instance 'loginClient' of type 'loginClient'
    {
        ...
        ports[0] = getDataPacketID((xme_core_component_t)2, 0);
        ports[1] = getDataPacketID((xme_core_component_t)2, 1);
        ports[2] = getDataPacketID((xme_core_component_t)2, 2);
...
createWaypoints(void)
{
...

    {
        ...
        if (NULL != demarshalerWaypointDescriptor)
        {
            ...
            (
                demarshalerWaypointDescriptor,
                &ports[12], // demarshaler.configuration[0].in (pnpClient.inGraph)
                1,
                &ports[10], // demarshaler.configuration[0].out (pnpClient.inGraph)
            ...
            (
                demarshalerWaypointDescriptor,
                &ports[13], // demarshaler.configuration[1].in (loginClient.inResponse)
                1,
                &ports[11], // demarshaler.configuration[1].out (loginClient.inResponse)
        ...
        {
            {
                ...
                (
                    udpReceiveWaypointDescriptor,
                    &ports[14], // udpReceive.configuration[0].out (pnpClient.inGraph)
                    key,
                    33225,
                    XME_CORE_TOPIC_PNPMANAGER_RUNTIME_GRAPH_MODEL,
                    sizeof(xme_core_topic_pnpManager_runtime_graph_model_t),
            ...
            }
            {
                ...
                (
                    udpReceiveWaypointDescriptor,
                    &ports[15], // udpReceive.configuration[1].out (loginClient.inResponse)
                    key,
                    33225,
                    XME_CORE_TOPIC_LOGIN_LOGINRESPONSE,
                    sizeof(xme_core_topic_login_loginResponse_t),
...
addDataPacketTransferEntries(void)
{
    ...
    status = xme_core_broker_addDataPacketTransferEntry(ports[1], ports[17]); // loginClient.outRequest -> marshaler.configuration[0].in (loginClient.outRequest)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[1], ports[17], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[2], ports[4]); // loginClient.outLoginAcknowledgment -> pnpClient.inLoginAcknowledgment
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[2], ports[4], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[10], ports[5]); // demarshaler.configuration[0].out (pnpClient.inGraph) -> pnpClient.inGraph
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[10], ports[5], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[11], ports[0]); // demarshaler.configuration[1].out (loginClient.inResponse) -> loginClient.inResponse
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[11], ports[0], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[14], ports[12]); // udpReceive.configuration[0].out (pnpClient.inGraph) -> demarshaler.configuration[0].in (pnpClient.inGraph)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[14], ports[12], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[15], ports[13]); // udpReceive.configuration[1].out (loginClient.inResponse) -> demarshaler.configuration[1].in (loginClient.inResponse)
...
main(int argc, char* argv[])
{
    ...
    xme_hal_env_setConsoleTitle("lonelyNode");
    ...
    // PROTECTED REGION ID(LONELYNODE_MAIN_BEFORE_COMPONENT_CREATION) ENABLED START
    ...
    // PROTECTED REGION ID(LONELYNODE_MAIN_RUN_BEFORE) ENABLED START
    ...
    // PROTECTED REGION ID(LONELYNODE_MAIN_RUN_AFTER) ENABLED START
    ...
    XME_LOG(XME_LOG_NOTE, "Node lonelyNode (nodeID %d) has shut down.\n", xme_core_node_getCurrentNodeId());
