 ...
 *
 * $Id: sensorKBManifest.c 7769 2014-03-11 15:27:13Z geisinger $
 */
...
#include "../include/sensorKBComponentWrapper.h"
...
/******************************************************************************/
xme_status_t
sensorMonitor_adv_sensorKB_manifest_createComponentTypeManifest
(
...
)
{
    ...
    componentManifest->componentType = (xme_core_componentType_t)4355;
    componentManifest->componentWrapperInit = sensorMonitor_adv_sensorKB_sensorKBComponentWrapper_init;
    componentManifest->componentWrapperReceivePort = sensorMonitor_adv_sensorKB_sensorKBComponentWrapper_receivePort;
    componentManifest->componentWrapperFini = sensorMonitor_adv_sensorKB_sensorKBComponentWrapper_fini;
    componentManifest->componentInit = (xme_core_componentManifest_componentInit_t)sensorMonitor_adv_sensorKB_sensorKBComponent_init;
    (void)xme_hal_safeString_strncpy(componentManifest->name, "sensorKB", sizeof(componentManifest->name));
    componentManifest->configStructSize = sizeof(sensorMonitor_adv_sensorKB_sensorKBComponent_config_t);
        ...
        // Function 'readSensorValue'
        {
            if (0U >= functionArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0U == functionArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'sensorKB' defines more functions (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                       1,
                       functionArrayLength
            ...
            else
            {
                ...
                functionManifest->requiredPortIndices[0] = (sensorMonitor_adv_sensorKB_sensorKBComponentWrapper_internalPortId_t)SENSORMONITOR_ADV_SENSORKB_SENSORKBCOMPONENTWRAPPER_PORT_SENSORVALUEOUT;
                functionManifest->optionalPortIndicesLength = 0;
                functionManifest->functionInit = (xme_core_exec_initCallback_t)sensorMonitor_adv_sensorKB_readSensorValueFunction_init;
                functionManifest->functionFini = (xme_core_exec_finiCallback_t)sensorMonitor_adv_sensorKB_readSensorValueFunction_fini;
                functionManifest->functionWrapperExecute = sensorMonitor_adv_sensorKB_readSensorValueFunctionWrapper_execute;
            }
        }
    }
        ...
        // Publication 'sensorValueOut'
        {
            if (0 >= portArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0 == portArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'sensorKB' defines more ports (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                        1,
                        portArrayLength
            ...
            else
            {
                ...
                // Add attribute definition: unit = SENSORMONITOR_ATTRIBUTE_UNIT_KILOBYTES
                {
                    uint32_t value = SENSORMONITOR_ATTRIBUTE_UNIT_KILOBYTES;
