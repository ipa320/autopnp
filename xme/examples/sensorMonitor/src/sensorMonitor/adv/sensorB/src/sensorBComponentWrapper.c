 ...
 *
 * $Id: sensorBComponentWrapper.c 7774 2014-03-11 17:38:48Z geisinger $
 */
...
static outputPort_t outputPorts[] = {
    {XME_CORE_DATAMANAGER_DATAPACKETID_INVALID, { 0U } } // sensorB.sensorValueOut
};
...
sensorMonitor_adv_sensorB_sensorBComponentWrapper_completeWriteOperations(void)
{
    ...
    for (outputPortIndex = 0U; outputPortIndex < outputPortCount; outputPortIndex++)
    {
        ...
        (void) sensorMonitor_adv_sensorB_sensorBComponentWrapper_writeNextPacket(outputPortIndex);
    }
}
...
)
{
    ...
    outputPortIndex = ((uint8_t)SENSORMONITOR_ADV_SENSORB_SENSORBCOMPONENTWRAPPER_PORT_SENSORVALUEOUT);
    ...
    else
    {
        ...
        XME_LOG(XME_LOG_ERROR,
            "[sensorBComponentWrapper] writePortSensorValueOut(): Writing to port failed. Data Handler returned error code %d.\n",
            status);
    }
...
)
{
    ...
    else
    {
        ...
        (
            XME_LOG_ERROR,
            "%s:%d sensorMonitor_adv_sensorB_sensorBComponentWrapper_writeOutputPortAttribute: Writing to port failed. Data Handler returned error code %d.",
            __FILE__,
            __LINE__,
            status
        );
...
)
{
...
#endif // #ifdef XME_MULTITHREAD
    {
        ...
        (
            ...
            XME_LOG_ERROR,
            "[sensorBComponentWrapper] CompleteWriteOperation for port (interalPortId: %d, dataPacketId: %d) returned error code %d.\n",
        ...
        if (XME_STATUS_BUFFER_TOO_SMALL == status)
        {
            ...
            (
                XME_LOG_WARNING,
                "[sensorBComponentWrapper] Data loss on port (interalPortId: %d, dataPacketId: %d), because of full buffer. Oldest value was overwritten.\n",
