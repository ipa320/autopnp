 ...
 *
 * $Id: sensorMBComponentWrapper.c 7774 2014-03-11 17:38:48Z geisinger $
 */
...
static outputPort_t outputPorts[] = {
    {XME_CORE_DATAMANAGER_DATAPACKETID_INVALID, { 0U } } // sensorMB.sensorValueOut
};
...
sensorMonitor_adv_sensorMB_sensorMBComponentWrapper_completeWriteOperations(void)
{
    ...
    for (outputPortIndex = 0U; outputPortIndex < outputPortCount; outputPortIndex++)
    {
        ...
        (void) sensorMonitor_adv_sensorMB_sensorMBComponentWrapper_writeNextPacket(outputPortIndex);
    }
}
...
)
{
    ...
    outputPortIndex = ((uint8_t)SENSORMONITOR_ADV_SENSORMB_SENSORMBCOMPONENTWRAPPER_PORT_SENSORVALUEOUT);
    ...
    else
    {
        ...
        XME_LOG(XME_LOG_ERROR,
            "[sensorMBComponentWrapper] writePortSensorValueOut(): Writing to port failed. Data Handler returned error code %d.\n",
            status);
    }
...
)
{
    ...
    else
    {
        ...
        (
            XME_LOG_ERROR,
            "%s:%d sensorMonitor_adv_sensorMB_sensorMBComponentWrapper_writeOutputPortAttribute: Writing to port failed. Data Handler returned error code %d.",
            __FILE__,
            __LINE__,
            status
        );
...
)
{
...
#endif // #ifdef XME_MULTITHREAD
    {
        ...
        (
            ...
            XME_LOG_ERROR,
            "[sensorMBComponentWrapper] CompleteWriteOperation for port (interalPortId: %d, dataPacketId: %d) returned error code %d.\n",
        ...
        if (XME_STATUS_BUFFER_TOO_SMALL == status)
        {
            ...
            (
                XME_LOG_WARNING,
                "[sensorMBComponentWrapper] Data loss on port (interalPortId: %d, dataPacketId: %d), because of full buffer. Oldest value was overwritten.\n",
