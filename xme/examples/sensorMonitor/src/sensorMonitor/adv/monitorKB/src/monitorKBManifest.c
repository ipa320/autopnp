 ...
 *
 * $Id: monitorKBManifest.c 7769 2014-03-11 15:27:13Z geisinger $
 */
...
#include "../include/monitorKBComponentWrapper.h"
...
/******************************************************************************/
xme_status_t
sensorMonitor_adv_monitorKB_manifest_createComponentTypeManifest
(
...
)
{
    ...
    componentManifest->componentType = (xme_core_componentType_t)4362;
    componentManifest->componentWrapperInit = sensorMonitor_adv_monitorKB_monitorKBComponentWrapper_init;
    componentManifest->componentWrapperReceivePort = sensorMonitor_adv_monitorKB_monitorKBComponentWrapper_receivePort;
    componentManifest->componentWrapperFini = sensorMonitor_adv_monitorKB_monitorKBComponentWrapper_fini;
    componentManifest->componentInit = (xme_core_componentManifest_componentInit_t)sensorMonitor_adv_monitorKB_monitorKBComponent_init;
    (void)xme_hal_safeString_strncpy(componentManifest->name, "monitorKB", sizeof(componentManifest->name));
    componentManifest->configStructSize = sizeof(sensorMonitor_adv_monitorKB_monitorKBComponent_config_t);
        ...
        // Function 'printSensorValue'
        {
            if (0U >= functionArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0U == functionArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'monitorKB' defines more functions (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                       1,
                       functionArrayLength
            ...
            else
            {
                ...
                functionManifest->requiredPortIndices[0] = (sensorMonitor_adv_monitorKB_monitorKBComponentWrapper_internalPortId_t)SENSORMONITOR_ADV_MONITORKB_MONITORKBCOMPONENTWRAPPER_PORT_SENSORVALUEIN;
                functionManifest->optionalPortIndicesLength = 0;
                functionManifest->functionInit = (xme_core_exec_initCallback_t)sensorMonitor_adv_monitorKB_printSensorValueFunction_init;
                functionManifest->functionFini = (xme_core_exec_finiCallback_t)sensorMonitor_adv_monitorKB_printSensorValueFunction_fini;
                functionManifest->functionWrapperExecute = sensorMonitor_adv_monitorKB_printSensorValueFunctionWrapper_execute;
            }
        }
    }
        ...
        // Subscription 'sensorValueIn'
        {
            if (0 >= portArrayLength) // Check generated by tool (which does not know about the array size)
            {
                if (0 == portArrayLength) // Only trigger warning once
                {
                    ...
                    (
                        XME_LOG_WARNING,
                        "%s:%d Component type 'monitorKB' defines more ports (%d) than can be stored in the manifest data structure (%d).\n",
                        __FILE__,
                        __LINE__,
                        1,
                        portArrayLength
            ...
            else
            {
                ...
                // Add attribute filter: unit == SENSORMONITOR_ATTRIBUTE_UNIT_KILOBYTES
                {
