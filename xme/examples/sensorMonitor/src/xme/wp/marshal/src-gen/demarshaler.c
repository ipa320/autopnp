 ...
 *
 * $Id: demarshaler.c 7621 2014-02-28 08:57:42Z ruiz $
 */
...
/**
 * \brief  Performs demarshaling for topic 'sensorData'.
 *
...
)
{
    ...
    if (SENSORMONITOR_TOPIC_SENSORDATA == topic)
    {
        ...
        buffer = &topicData;
        bufferSize = sizeof(sensorMonitor_topic_sensorData_t);
        
        status = doDemarshalingForSensorData
        (
            inputPort,
            outputPort,
            &topicData
...
{
    const uint32_t marshaledDataSize = 104;
    ...
    // char topicData->hostName
    {
        ...
        for (i0 = 0; i0 < 32; i0++)
        {
            // char topicData->hostName
            {
                ...
                topicData->hostName[i0] = hostValue;
    ...
    // char topicData->sensorType
    {
        ...
        for (i0 = 0; i0 < 32; i0++)
        {
            // char topicData->sensorType
            {
                ...
                hostValue = (char)(*(uint8_t*)bufferPtr);
                topicData->sensorType[i0] = hostValue;
                bufferPtr += sizeof(uint8_t);
            }
        }
    }
    ...
    // char topicData->sensorInstance
    {
        ...
        for (i0 = 0; i0 < 32; i0++)
        {
            // char topicData->sensorInstance
            {
                ...
                hostValue = (char)(*(uint8_t*)bufferPtr);
                topicData->sensorInstance[i0] = hostValue;
                bufferPtr += sizeof(uint8_t);
            }
        }
    }
    ...
    // uint64_t topicData->sensorValue
    {
        ...
        topicData->sensorValue = hostValue;
        bufferPtr += 8;
    }
    ...
    {
        sensorMonitor_attribute_unit_t demarshaledAttribute;
        uint8_t marshaledAttribute[4];
        uint8_t* bufferPtr;
