 ...
 *
 * $Id: marshaler.c 7621 2014-02-28 08:57:42Z ruiz $
 */
...
/**
 * \brief  Performs marshaling for topic 'sensorData'.
 *
...
)
{
    ...
    if (SENSORMONITOR_TOPIC_SENSORDATA == topic)
    {
        uint8_t marshaledData[104];
        
        buffer = marshaledData;
        bufferSize = 104;
        
        status = doMarshalingForSensorData
        (
            inputPort,
            outputPort,
            (void*)marshaledData
...
{
    sensorMonitor_topic_sensorData_t topicData;
    ...
    topicDataSize = sizeof(sensorMonitor_topic_sensorData_t);
    ...
    // char topicData.hostName
    {
        uint8_t i0;
        
        for (i0 = 0; i0 < 32; i0++)
        {
            // char topicData.hostName[i0]
            xme_hal_mem_copy(bufferPtr, &topicData.hostName[i0], sizeof(char));
            bufferPtr += sizeof(char);
        }
    }
    
    // char topicData.sensorType
    {
        uint8_t i0;
        
        for (i0 = 0; i0 < 32; i0++)
        {
            // char topicData.sensorType[i0]
            xme_hal_mem_copy(bufferPtr, &topicData.sensorType[i0], sizeof(char));
            bufferPtr += sizeof(char);
        }
    }
    
    // char topicData.sensorInstance
    {
        uint8_t i0;
        
        for (i0 = 0; i0 < 32; i0++)
        {
            // char topicData.sensorInstance[i0]
            xme_hal_mem_copy(bufferPtr, &topicData.sensorInstance[i0], sizeof(char));
            bufferPtr += sizeof(char);
        }
    }
    
    // uint64_t topicData.sensorValue
    {
        uint64_t netValue;
    
        netValue = xme_hal_net_htonll((uint64_t)topicData.sensorValue);
    ...
    // sensorMonitor_attribute_unit_t unit
    {
        sensorMonitor_attribute_unit_t attributeValue;
        uint8_t marshaledAttribute[4];
        uint8_t* bufferPtr;
        ...
        (
            inputPort,
            XME_CORE_ATTRIBUTES(SENSORMONITOR_ATTRIBUTE_UNIT),
            &attributeValue,
            sizeof(attributeValue),
            &bytesRead
        );
        ...
        XME_CHECK(XME_STATUS_SUCCESS == status || XME_STATUS_NO_SUCH_VALUE == status, XME_STATUS_INTERNAL_ERROR);
        XME_CHECK(XME_STATUS_NO_SUCH_VALUE == status || bytesRead == sizeof(sensorMonitor_attribute_unit_t), XME_STATUS_INTERNAL_ERROR);
