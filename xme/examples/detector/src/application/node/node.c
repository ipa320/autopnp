 ...
 * \details Index | Port
 *              0 | logger.info
 *              1 | pnpClient.inLoginAcknowledgment
 *              2 | pnpClient.inGraph
 *              3 | pnpClient.outManifest
 *              4 | pnpClient.outLogoutAcknowledgment
 *              5 | pnpClient.outLogoutRequest
 *              6 | pnpClient.outRemoveComponentRequest
 *              7 | pnpManager.inComponentInstanceManifest
 *              8 | pnpManager.inPnPLoginRequest
 *              9 | pnpManager.inLogoutRequest
 *             10 | pnpManager.inLogoutAcknowledgment
 *             11 | pnpManager.inRemoveComponentRequest
 *             12 | pnpManager.outRuntimeGraph
 *             13 | pnpManager.outPnPLoginResponse
 *             14 | pnpManager.outLogoutNotification
 *             15 | demarshaler.configuration[0].out (pnpManager.inLogoutRequest)
 *             16 | demarshaler.configuration[1].out (pnpManager.inLogoutAcknowledgment)
 *             17 | demarshaler.configuration[2].out (pnpClient.inGraph)
 *             18 | demarshaler.configuration[0].in (pnpManager.inLogoutRequest)
 *             19 | demarshaler.configuration[1].in (pnpManager.inLogoutAcknowledgment)
 *             20 | demarshaler.configuration[2].in (pnpClient.inGraph)
 *             21 | udpReceive.configuration[0].out (pnpManager.inLogoutRequest)
 *             22 | udpReceive.configuration[1].out (pnpManager.inLogoutAcknowledgment)
 *             23 | udpReceive.configuration[2].out (pnpClient.inGraph)
 */
static xme_core_dataManager_dataPacketId_t ports[24];

/**
 * \brief Buffers required by UdpReceiveWaypoint.
 */
static void* udpReceiveBuffers[3];
static xme_core_exec_functionDescriptor_t* detectorFuncDescs[1] = {NULL};
static xme_core_exec_functionDescriptor_t* loggerFuncDescs[1] = {NULL};
...
static xme_wp_waypoint_instanceId_t demarshalerWaypointInstanceIds[3] = { XME_WP_WAYPOINT_INSTANCEID_INVALID, XME_WP_WAYPOINT_INSTANCEID_INVALID, XME_WP_WAYPOINT_INSTANCEID_INVALID };
static xme_core_exec_functionDescriptor_t* udpReceiveWaypointDescriptor = NULL;
static xme_wp_waypoint_instanceId_t udpReceiveWaypointInstanceIds[3] = { XME_WP_WAYPOINT_INSTANCEID_INVALID, XME_WP_WAYPOINT_INSTANCEID_INVALID, XME_WP_WAYPOINT_INSTANCEID_INVALID };
...
)
{
    ...
    // PROTECTED REGION END
     ...
     // Add interface to node registry
     {
         ...
         (void)xme_hal_safeString_strncpy(addressBuffer, "127.0.0.1:65000", sizeof(addressBuffer));
...
pnpInit(void)
{
    ...
    announceComponentPortsToPnpClient((xme_core_component_t)128); // Component 'detector' of type 'detector'
    announceComponentPortsToPnpClient((xme_core_component_t)129); // Component 'logger' of type 'logger'
    ...
    // Register node 'node'
    {
        ...
        status = xme_core_directory_nodeRegistryController_registerNode((xme_core_node_nodeId_t)1, "node", (xme_core_node_guid_t)1);
        XME_CHECK_MSG
        (
    ...
    (
        ...
        (xme_core_component_t)3, // from node 'node' component 'pnpManager'
        5, // publication 'outRuntimeGraph'
        (xme_core_node_nodeId_t)1,
        (xme_core_component_t)4, // to node 'node' component 'pnpClient'
    ...
    (
        (xme_core_node_nodeId_t)1,
        (xme_core_component_t)4, // from node 'node' component 'pnpClient'
        2, // publication 'outManifest'
        (xme_core_node_nodeId_t)1,
        (xme_core_component_t)3, // to node 'node' component 'pnpManager'
    ...
    // Node 'node', component 'pnpClient' of type 'pnpClient'
    status = xme_core_pnp_pnpManager_announcePorts
    (
        (xme_core_node_nodeId_t)1,
    ...
    // Node 'node', component 'pnpManager' of type 'pnpManager'
    status = xme_core_pnp_pnpManager_announcePorts
    (
        (xme_core_node_nodeId_t)1,
...
configureSchedule(void)
{
    ...
    // Component 'detector' function 'detectDevices'
    if (NULL != detectorFuncDescs[0])
    {
        status = xme_core_exec_dispatcher_createFunctionExecutionUnit(detectorFuncDescs[0], true);
        XME_ASSERT_NORVAL(XME_STATUS_SUCCESS == status);
    }
    // Component 'logger' function 'print'
    if (NULL != loggerFuncDescs[0])
    {
        status = xme_core_exec_dispatcher_createFunctionExecutionUnit(loggerFuncDescs[0], true);
        XME_ASSERT_NORVAL(XME_STATUS_SUCCESS == status);
    }
    ...
    (
        &(nodeSchedules[0]),
        xme_hal_time_timeIntervalFromMilliseconds(350ull)
    );
    ...
    if (NULL != detectorFuncDescs[0])
    {
        // Add function 'detectDevices' of component instance 'detector' of type 'detector' to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
            detectorFuncDescs[0]->componentId,
            detectorFuncDescs[0]->functionId,
            detectorFuncDescs[0]->taskArgs,
    ...
    if (NULL != loggerFuncDescs[0])
    {
        // Add function 'print' of component instance 'logger' of type 'logger' to schedule
        status = xme_core_exec_scheduler_addElementToScheduleTable
        (
            nodeSchedules[0],
            loggerFuncDescs[0]->componentId,
            loggerFuncDescs[0]->functionId,
            loggerFuncDescs[0]->taskArgs,
...
createComponentInstances(void)
{
    ...
    // Create statically configured component instance 'detector' of type 'detector'
    {
        ...
        XME_CHECK_MSG(NULL != builder, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'detector' of type 'detector' failed. Builder could not be created.\n", __FILE__, __LINE__);
        xme_core_nodeMgr_compRep_builderSetComponentID(builder, (xme_core_component_t)128);
        xme_core_nodeMgr_compRep_builderSetExecutionPeriod(builder, 0, xme_hal_time_timeIntervalFromMilliseconds(2100ull)); // Function 'detectDevices'
        status = xme_core_nodeMgr_compRep_build(builder, &componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'detector' of type 'detector' failed. Building of component failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_setStateToAnnounced(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'detector' of type 'detector' failed. Setting state to ANNOUNCED failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_createAndRegisterComponent(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'detector' of type 'detector' failed. Registration and port allocation failed.\n", __FILE__, __LINE__);

        
        detectorFuncDescs[0] = getFunctionDescriptor((xme_core_component_t)128, 0);
    }
    
    // Create statically configured component instance 'logger' of type 'logger'
    {
        xme_core_nodeMgr_compRep_componentBuilder_t* builder = NULL;
        xme_core_nodeMgr_compRep_componentHandle_t componentHandle;

        builder = xme_core_nodeMgr_compRep_createBuilder(XME_CORE_NODE_LOCAL_NODE_ID, (xme_core_componentType_t)4099);
        XME_CHECK_MSG(NULL != builder, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'logger' of type 'logger' failed. Builder could not be created.\n", __FILE__, __LINE__);
        xme_core_nodeMgr_compRep_builderSetComponentID(builder, (xme_core_component_t)129);
        xme_core_nodeMgr_compRep_builderSetExecutionPeriod(builder, 0, xme_hal_time_timeIntervalFromMilliseconds(350ull)); // Function 'print'
        status = xme_core_nodeMgr_compRep_build(builder, &componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'logger' of type 'logger' failed. Building of component failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_setStateToAnnounced(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'logger' of type 'logger' failed. Setting state to ANNOUNCED failed.\n", __FILE__, __LINE__);

        status = xme_core_nodeMgr_compRep_createAndRegisterComponent(componentHandle);
        XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Creation of component instance 'logger' of type 'logger' failed. Registration and port allocation failed.\n", __FILE__, __LINE__);

        ports[0] = getDataPacketID((xme_core_component_t)129, 0);
        
        loggerFuncDescs[0] = getFunctionDescriptor((xme_core_component_t)129, 0);
    }
    ...
    // Create statically configured component instance 'pnpManager' of type 'pnpManager'
    {
        ...
        ports[7] = getDataPacketID((xme_core_component_t)3, 0);
        ports[8] = getDataPacketID((xme_core_component_t)3, 1);
        ports[9] = getDataPacketID((xme_core_component_t)3, 2);
        ports[10] = getDataPacketID((xme_core_component_t)3, 3);
        ports[11] = getDataPacketID((xme_core_component_t)3, 4);
        ports[12] = getDataPacketID((xme_core_component_t)3, 5);
        ports[13] = getDataPacketID((xme_core_component_t)3, 6);
        ports[14] = getDataPacketID((xme_core_component_t)3, 7);
    ...
    // Create statically configured component instance 'pnpClient' of type 'pnpClient'
    {
        ...
        ports[1] = getDataPacketID((xme_core_component_t)4, 0);
        ports[2] = getDataPacketID((xme_core_component_t)4, 1);
        ports[3] = getDataPacketID((xme_core_component_t)4, 2);
        ports[4] = getDataPacketID((xme_core_component_t)4, 3);
        ports[5] = getDataPacketID((xme_core_component_t)4, 4);
        ports[6] = getDataPacketID((xme_core_component_t)4, 5);
...
createWaypoints(void)
{
...

    {
        ...
        if (NULL != demarshalerWaypointDescriptor)
        {
            ...
            (
                demarshalerWaypointDescriptor,
                &ports[18], // demarshaler.configuration[0].in (pnpManager.inLogoutRequest)
                1,
                &ports[15], // demarshaler.configuration[0].out (pnpManager.inLogoutRequest)
            ...
            (
                demarshalerWaypointDescriptor,
                &ports[19], // demarshaler.configuration[1].in (pnpManager.inLogoutAcknowledgment)
                1,
                &ports[16], // demarshaler.configuration[1].out (pnpManager.inLogoutAcknowledgment)
            ...
            (
                demarshalerWaypointDescriptor,
                &ports[20], // demarshaler.configuration[2].in (pnpClient.inGraph)
                1,
                &ports[17], // demarshaler.configuration[2].out (pnpClient.inGraph)
        ...
        {
            {
                ...
                (
                    udpReceiveWaypointDescriptor,
                    &ports[21], // udpReceive.configuration[0].out (pnpManager.inLogoutRequest)
                    key,
                    65000,
                    XME_CORE_TOPIC_PNP_LOGOUTREQUEST,
                    sizeof(xme_core_topic_pnp_logoutRequest_t),
            ...
            }
            {
                ...
                (
                    udpReceiveWaypointDescriptor,
                    &ports[22], // udpReceive.configuration[1].out (pnpManager.inLogoutAcknowledgment)
                    key,
                    65000,
                    XME_CORE_TOPIC_PNP_LOGOUTACKNOWLEDGMENT,
                    sizeof(xme_core_topic_pnp_logoutAcknowledgment_t),
            ...
            }
            {
                ...
                (
                    udpReceiveWaypointDescriptor,
                    &ports[23], // udpReceive.configuration[2].out (pnpClient.inGraph)
                    key,
                    65000,
                    XME_CORE_TOPIC_PNPMANAGER_RUNTIME_GRAPH_MODEL,
                    sizeof(xme_core_topic_pnpManager_runtime_graph_model_t),
...
registerTopicsAndAttributes(void)
{
    ...
    // Register topic data size for topic 'device_info'
    {
        xme_status_t status;
        ...
        (
            ...
            sizeof(detector_topic_device_info_t),
            false
        );
    ...
    // Register attribute descriptor list for topic 'device_info'
    {
        xme_status_t status;
        xme_core_attribute_descriptor_list_t attributeDescriptorList;
        xme_core_attribute_descriptor_t attributeDescriptors[1];
        ...
        if (status != XME_STATUS_SUCCESS)
        {
            XME_LOG(XME_LOG_WARNING, "Registration of attribute descriptor list for topic 'device_info' failed.\n");
        }
    }
...
createComponentTypeManifests(void)
{
    ...
    // Create and add manifest for component type 'detector' to manifest repository
    {
        xme_core_componentManifest_t componentManifest;

        status = detector_adv_detector_manifest_createComponentTypeManifest(&componentManifest);

        if (XME_STATUS_SUCCESS != status)
        {
            XME_LOG(XME_LOG_WARNING, "Creation of component type manifest for 'detector' failed.\n");
        }
        else
        {
            ...
            if (XME_STATUS_SUCCESS != status)
            {
                XME_LOG(XME_LOG_WARNING, "Addtion of component type manifest to manifest repository failed for 'detector'.\n");
            }
        }
    }
    
    // Create and add manifest for component type 'device' to manifest repository
    {
        xme_core_componentManifest_t componentManifest;

        status = detector_adv_device_manifest_createComponentTypeManifest(&componentManifest);

        if (XME_STATUS_SUCCESS != status)
        {
            XME_LOG(XME_LOG_WARNING, "Creation of component type manifest for 'device' failed.\n");
        }
        else
        {
            status = xme_core_manifestRepository_addComponentManifest(&componentManifest, false);
            ...
            if (XME_STATUS_SUCCESS != status)
            {
                XME_LOG(XME_LOG_WARNING, "Addtion of component type manifest to manifest repository failed for 'device'.\n");
            }
        }
    }
    
    // Create and add manifest for component type 'logger' to manifest repository
    {
        xme_core_componentManifest_t componentManifest;

        status = detector_adv_logger_manifest_createComponentTypeManifest(&componentManifest);

        if (XME_STATUS_SUCCESS != status)
        {
            XME_LOG(XME_LOG_WARNING, "Creation of component type manifest for 'logger' failed.\n");
        }
        else
        {
            status = xme_core_manifestRepository_addComponentManifest(&componentManifest, false);
            ...
            if (XME_STATUS_SUCCESS != status)
            {
                XME_LOG(XME_LOG_WARNING, "Addtion of component type manifest to manifest repository failed for 'logger'.\n");
            }
        }
    }
...
registerComponentTypesInLoggingEngine(void)
{
    ...
    {
        // Register component type 'detector' in logging engine
        status = xme_core_log_registerComponent((xme_core_log_componentID_t) 4097, "detector", xme_core_log_getMinimumLogSeverityLevel());
        XME_LOG_IF
        (
            XME_STATUS_SUCCESS != status,
            XME_LOG_WARNING,
    ...
    {
        // Register component type 'device' in logging engine
        status = xme_core_log_registerComponent((xme_core_log_componentID_t) 4098, "device", xme_core_log_getMinimumLogSeverityLevel());
        XME_LOG_IF
        (
            XME_STATUS_SUCCESS != status,
            XME_LOG_WARNING,
    ...
    {
        // Register component type 'logger' in logging engine
        status = xme_core_log_registerComponent((xme_core_log_componentID_t) 4099, "logger", xme_core_log_getMinimumLogSeverityLevel());
        XME_LOG_IF
        (
            XME_STATUS_SUCCESS != status,
            XME_LOG_WARNING,
...
addDataPacketTransferEntries(void)
{
    ...
    status = xme_core_broker_addDataPacketTransferEntry(ports[3], ports[7]); // pnpClient.outManifest -> pnpManager.inComponentInstanceManifest
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[3], ports[7], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[4], ports[10]); // pnpClient.outLogoutAcknowledgment -> pnpManager.inLogoutAcknowledgment
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[4], ports[10], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[5], ports[9]); // pnpClient.outLogoutRequest -> pnpManager.inLogoutRequest
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[5], ports[9], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[6], ports[11]); // pnpClient.outRemoveComponentRequest -> pnpManager.inRemoveComponentRequest
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[6], ports[11], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[12], ports[2]); // pnpManager.outRuntimeGraph -> pnpClient.inGraph
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[12], ports[2], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[15], ports[9]); // demarshaler.configuration[0].out (pnpManager.inLogoutRequest) -> pnpManager.inLogoutRequest
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[15], ports[9], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[16], ports[10]); // demarshaler.configuration[1].out (pnpManager.inLogoutAcknowledgment) -> pnpManager.inLogoutAcknowledgment
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[16], ports[10], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[17], ports[2]); // demarshaler.configuration[2].out (pnpClient.inGraph) -> pnpClient.inGraph
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[17], ports[2], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[21], ports[18]); // udpReceive.configuration[0].out (pnpManager.inLogoutRequest) -> demarshaler.configuration[0].in (pnpManager.inLogoutRequest)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[21], ports[18], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[22], ports[19]); // udpReceive.configuration[1].out (pnpManager.inLogoutAcknowledgment) -> demarshaler.configuration[1].in (pnpManager.inLogoutAcknowledgment)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[22], ports[19], status);
    status = xme_core_broker_addDataPacketTransferEntry(ports[23], ports[20]); // udpReceive.configuration[2].out (pnpClient.inGraph) -> demarshaler.configuration[2].in (pnpClient.inGraph)
    XME_CHECK_MSG(XME_STATUS_SUCCESS == status, XME_STATUS_INTERNAL_ERROR, XME_LOG_FATAL, "%s:%d Call to xme_core_broker_addDataPacketTransferEntry() failed for source data packet ID %d and destination data packet ID %d with status %d.\n", __FILE__, __LINE__, ports[23], ports[20], status);
