 ...
 *
 * $Id: senderComponentWrapper.c 7774 2014-03-11 17:38:48Z geisinger $
 */
...
static outputPort_t outputPorts[] = {
    {XME_CORE_DATAMANAGER_DATAPACKETID_INVALID, { 0U } } // sender.out
};
...
queues_adv_sender_senderComponentWrapper_completeWriteOperations(void)
{
    ...
    for (outputPortIndex = 0U; outputPortIndex < outputPortCount; outputPortIndex++)
    {
        ...
        (void) queues_adv_sender_senderComponentWrapper_writeNextPacket(outputPortIndex);
    }
}
...
)
{
    ...
    outputPortIndex = ((uint8_t)QUEUES_ADV_SENDER_SENDERCOMPONENTWRAPPER_PORT_OUT);
    ...
    (
        ...
        (void*)data,
        (uint32_t)sizeof(queues_topic_data_t)
    );
    ...
    else
    {
        ...
        XME_LOG(XME_LOG_ERROR,
            "[senderComponentWrapper] writePortOut(): Writing to port failed. Data Handler returned error code %d.\n",
            status);
    }
...
)
{
    ...
    else
    {
        ...
        (
            XME_LOG_ERROR,
            "%s:%d queues_adv_sender_senderComponentWrapper_writeOutputPortAttribute: Writing to port failed. Data Handler returned error code %d.",
            __FILE__,
            __LINE__,
            status
        );
...
)
{
...
#endif // #ifdef XME_MULTITHREAD
    {
        ...
        (
            ...
            XME_LOG_ERROR,
            "[senderComponentWrapper] CompleteWriteOperation for port (interalPortId: %d, dataPacketId: %d) returned error code %d.\n",
        ...
        if (XME_STATUS_BUFFER_TOO_SMALL == status)
        {
            ...
            (
                XME_LOG_WARNING,
                "[senderComponentWrapper] Data loss on port (interalPortId: %d, dataPacketId: %d), because of full buffer. Oldest value was overwritten.\n",
