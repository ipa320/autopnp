...
/**
 * \brief  Performs marshaling for topic 'data'.
 *
...
)
{
    ...
    topicDataSize = sizeof(queues_topic_data_t);
    ...
    // uint32_t topicData.value
    {
        uint32_t netValue;
    
        netValue = xme_hal_net_htonl((uint32_t)topicData.value);
    ...
    // queues_attribute_sender_t sender
    {
        queues_attribute_sender_t attributeValue;
        uint8_t marshaledAttribute[1];
        uint8_t* bufferPtr;
        ...
        (
            inputPort,
            XME_CORE_ATTRIBUTES(QUEUES_ATTRIBUTE_SENDER),
            &attributeValue,
            sizeof(attributeValue),
            &bytesRead
        );
        ...
        XME_CHECK(XME_STATUS_SUCCESS == status || XME_STATUS_NO_SUCH_VALUE == status, XME_STATUS_INTERNAL_ERROR);
        XME_CHECK(XME_STATUS_NO_SUCH_VALUE == status || bytesRead == sizeof(queues_attribute_sender_t), XME_STATUS_INTERNAL_ERROR);
    
        // uint8_t attributeValue
        {
            uint8_t netValue;
        
            netValue = (uint8_t)attributeValue;
