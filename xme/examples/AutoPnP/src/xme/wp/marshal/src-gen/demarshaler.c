...
/**
 * \brief  Performs demarshaling for topic 'add_component'.
 *
 ...
 */
xme_status_t
doDemarshalingForAdd_component
(
    xme_core_dataManager_dataPacketId_t inputPort,
    xme_core_dataManager_dataPacketId_t outputPort,
...
/**
 * \brief  Performs demarshaling for topic 'remove_component'.
 *
 * \param  inputPort Input port where marshaled data is stored.
 * \param  topicData Buffer where marshaled data will be written to.
 *
 ...
 */
xme_status_t
doDemarshalingForRemove_component
(
    xme_core_dataManager_dataPacketId_t inputPort,
    xme_core_dataManager_dataPacketId_t outputPort,
...
)
{
    ...
    if (AUTOPNP_TOPIC_ADD_COMPONENT == topic)
    {
        ...
        buffer = &topicData;
        bufferSize = sizeof(AutoPnP_topic_add_component_t);
        
        status = doDemarshalingForAdd_component
        (
            inputPort,
            outputPort,
            &topicData
    ...
    else if (AUTOPNP_TOPIC_REMOVE_COMPONENT == topic)
    {
        ...
        buffer = &topicData;
        bufferSize = sizeof(AutoPnP_topic_remove_component_t);
        
        status = doDemarshalingForRemove_component
        (
            inputPort,
            outputPort,
            &topicData
...
{
    const uint32_t marshaledDataSize = 601;
    ...
    // AutoPnP_capability_t topicData->capabilities
    {
        ...
        for (i0 = 0; i0 < 10; i0++)
        {
            for (i1 = 0; i1 < 50; i1++)
            {
                // char topicData->capabilities[i0]
                {
                    char hostValue;
                
                    hostValue = (char)(*(uint8_t*)bufferPtr);
                    topicData->capabilities[i0][i1] = hostValue;
                    bufferPtr += sizeof(uint8_t);
                }
            }
        }
    ...
    // uint8_t topicData->capabilitiesCount
    {
        ...
        topicData->capabilitiesCount = hostValue;
        bufferPtr += 1;
    }
...
{
    const uint32_t marshaledDataSize = 50;
    unsigned int bytesRead;
