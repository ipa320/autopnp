...
/***   Includes                                                             ***/
/******************************************************************************/
#include "chromosomeGui/adv/componentOne/include/manifest.h"
...
)
{
    ...
    // Function 'doSomething'
    {
        if (0 >= functionArrayLength) // Check generated by tool (which does not know about the array size)
        {
            if (0 == functionArrayLength) // Only trigger warning once
            {
                ...
                (
                    XME_LOG_WARNING,
                    "%s:%d Component type 'componentOne' defines more functions (%d) than can be stored in the manifest data structure (%d).\n",
                    __FILE__,
                    __LINE__,
                    2,
                    functionArrayLength
    ...
    // Function 'publishText'
    {
        ...
        else
        {
            ...
            functionManifest->wcet = xme_hal_time_timeIntervalFromSeconds(0);
            functionManifest->alphaCurve.alphaCurve = 0;
            functionManifest->completion = true;
        }
    }
    ...
    // Subscription 'buttonPushed'
    {
        if (0 >= portArrayLength) // Check generated by tool (which does not know about the array size)
        {
            if (0 == portArrayLength) // Only trigger warning once
            {
                ...
                (
                    XME_LOG_WARNING,
                    "%s:%d Component type 'componentOne' defines more ports (%d) than can be stored in the manifest data structure (%d).\n",
                    __FILE__,
                    __LINE__,
                    2,
                    portArrayLength
