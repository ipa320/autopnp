    ...
    for (inputPortIndex = 0; inputPortIndex < inputPortCount; inputPortIndex++)
    {
        if (XME_CORE_COMPONENT_PORTSTATUS_VALID == inputPorts[inputPortIndex].status)
        {
            ...
            if (XME_STATUS_SUCCESS != status)
            {
                ...
                (
                    XME_LOG_ERROR,
                    "[componentOneComponentWrapper] CompleteReadOperation for port (interalPortId: %d, dataPacketId: %d) returned error code %d\n",
                    inputPortIndex,
    ...
    for (outputPortIndex = 0; outputPortIndex < outputPortCount; outputPortIndex++)
    {
        if (XME_CORE_COMPONENT_PORTSTATUS_VALID == outputPorts[outputPortIndex].status)
        {
            ...
            if (XME_STATUS_SUCCESS != status)
            {
                ...
                (
                    XME_LOG_ERROR,
                    "[componentOneComponentWrapper] CompleteWriteOperation for port (interalPortId: %d, dataPacketId: %d) returned error code %d\n",
...
)
{
    ...
    inputPortIndex = (uint8_t)CHROMOSOMEGUI_ADV_COMPONENTONE_COMPONENTONECOMPONENTWRAPPER_PORT_BUTTONPUSHED;
...
)
{
    ...
    outputPortIndex = ((uint8_t)CHROMOSOMEGUI_ADV_COMPONENTONE_COMPONENTONECOMPONENTWRAPPER_PORT_SENDTEXT) - inputPortCount;
    ...
    else
    {
        ...
        (
            XME_LOG_ERROR,
            "%s:%d chromosomeGui_adv_componentOne_componentOneComponentWrapper_writePortSendText: Error when writing to port. Data handler returned error code %d.",
            __FILE__,
            __LINE__,
            returnValue
        );
