 ...
 *
 * $Id: interfaceTestSync.cpp 5043 2013-09-11 16:45:41Z geisinger $
 */
...
{
    EXPECT_NO_XME_ASSERTION_FAILURES(EXPECT_EQ(XME_STATUS_INVALID_HANDLE, xme_hal_sync_tryEnterCriticalSection(XME_HAL_SYNC_INVALID_CRITICAL_SECTION_HANDLE)));
}

TEST_F(SyncInterfaceTest, tryEnterCriticalSectionWithArbitraryHandle)
{
    EXPECT_NO_XME_ASSERTION_FAILURES(EXPECT_EQ(XME_STATUS_INVALID_HANDLE, xme_hal_sync_tryEnterCriticalSection((xme_hal_sync_criticalSectionHandle_t) 42)));
}
