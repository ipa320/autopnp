 ...
 *
 * $Id: net.h 5058 2013-09-12 14:29:15Z geisinger $
 */
 ...
 *
 * \retval XME_CORE_STATUS_SUCCESS if the network abstraction has been properly initialized.
 * \retval XME_CORE_STATUS_OUT_OF_RESOURCES if network abstraction initialization failed.
 ...
 *
 * \retval XME_CORE_STATUS_SUCCESS if the network communication interface
 *                                 has been properly initialized.
 * \retval XME_HAL_STATUS_INVALID_INTERFACE if the given interface was
 *                                          invalid.
 * \retval XME_CORE_STATUS_OUT_OF_RESOURCES if not enough resources are
 ...
 *
 * \retval XME_CORE_STATUS_SUCCESS if the socket has been successfully closed and
 *                                 destroyed.
 * \retval XME_CORE_STATUS_INVALID_HANDLE if the given socket handle was invalid.
 */
xme_status_t
 ...
 *
 * \retval XME_CORE_STATUS_SUCCESS if the socket has been successfully
 *                                 opened or was already opened before.
 * \retval XME_CORE_STATUS_OPERATION_FAILED if the socket could not be
 *                                          opened.
 * \retval XME_CORE_STATUS_INVALID_HANDLE if the given socket handle was invalid.
 */
xme_status_t
 ...
 *
 * \retval XME_CORE_STATUS_SUCCESS if blocking behaviour was correctly established.
 * \retval XME_CORE_STATUS_INVALID_HANDLE if the given socket handle was invalid.
 * \retval XME_CORE_STATUS_INTERNAL_ERROR if the operation failed
 *                                        internally.
 */
xme_status_t
 ...
 * \retval XME_CORE_STATUS_SUCCESS if read or write operations are
 *                                 available.
 ...
 *
 * \retval XME_CORE_STATUS_SUCCESS if read or write operations are available.
 * \retval xme_status_tIMEOUT if the given timeout interval elapsed
 *                                 before read or write operations were available.
 * \retval XME_CORE_STATUS_INVALID_HANDLE if the given socket handle was invalid.
 * \retval XME_CORE_STATUS_INTERNAL_ERROR if the operation failed internally.
 */
xme_status_t
