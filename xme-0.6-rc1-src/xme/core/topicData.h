 ...
 *
 * $Id: topicData.h 5217 2013-09-27 16:26:43Z wiesmueller $
 */
...
typedef struct
{
    ...
    struct
    {
        ...
        char vertexData[256]; ///< Determines the vertex information:
                            ///< - If vertexType is labeled as waypoint, this field determines the vertex configuration data. 
                            ///< - If vertexType is labeled as component, this field determines the container (first two bytes) and the component (the following two bytes).
        xme_core_componentType_t componentType; ///< Component type.
        xme_core_component_t componentId; ///< Component identifier.
    }
    vertex[16]; ///< The definition of vertex in the graph. 
    struct
    {
    ...
    edge[16]; ///< The definition of edge in the graph.
}
...
typedef struct
{
    ...
    components[10]; ///< The list of components that are already running in the requestor node. 
}
