 ...
 *
 * $Id: configurator.c 5157 2013-09-24 16:29:43Z rupanov $
 */
...
)
{
    ...
    XME_LOG(XME_LOG_NOTE, MODULE_ACRONYM "completed addToSchedule() with status = %d\n",
            status);
...
{
    XME_LOG(XME_LOG_NOTE, MODULE_ACRONYM "entering transaction %d\n", transactionId);
    XME_CHECK_MSG(
        (transactionId == transactionDescriptor.transactionId)
        ||
