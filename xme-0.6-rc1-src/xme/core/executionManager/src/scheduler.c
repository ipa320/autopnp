 ...
 *
 * $Id: scheduler.c 5190 2013-09-26 13:10:19Z camek $
 */
...
/** \brief Increments the cycle counter for EM cycle control */
static xme_status_t
xme_core_exec_scheduler_incrementCycleCounterInt(void);

/**     odo document it too */
static xme_status_t
xme_core_exec_scheduler_incrementCycleCounterGlobal(void);
...
xme_core_exec_scheduler_getCycleCounter(void)
{
	...
	status = xme_core_dataHandler_readData(xme_core_exec_CycleCounter,
			&cycleCounterBuf, sizeof(xme_core_exec_CycleCounter), &bytesRead);
...
xme_core_exec_scheduler_incrementCycleCounterInt(void)
{
    XME_LOG(XME_LOG_VERBOSE, MODULE_ACRONYM
        "############################### CycleInt  %3"PRIu64" ######################################\n",
            ++xme_core_exec_scheduler_internalState.cycleCounter);

    XME_CHECK(XME_STATUS_SUCCESS ==  xme_core_exec_dispatcher_incCycleCount(),
              XME_STATUS_INTERNAL_ERROR);
...
xme_core_exec_scheduler_incrementCycleCounterGlobal(void)
{
    ...
    XME_CHECK(XME_STATUS_SUCCESS ==
        xme_core_dataHandler_readData(
                xme_core_exec_CycleCounter,
                &cycleCounterBuf,
                sizeof(xme_core_exec_CycleCounter),
                &bytesRead),
        XME_STATUS_INTERNAL_ERROR
    );
...
)
{
    ...
    *startDelay_ns = 0;
    ...
    do
    {
		...
		{
			// XXX make cycle counters run together, so that they are incremented sync. and processed together
			status = xme_core_exec_scheduler_incrementCycleCounterInt();
			XME_ASSERT(XME_STATUS_SUCCESS == status);
		...
		if(xme_core_exec_scheduler_internalState.nextTaskHandle == ((xme_core_exec_schedule_taskHandle_t)(numTasksInSchedule)))
		{
			...
			XME_CHECK(XME_STATUS_SUCCESS ==  xme_core_exec_scheduler_incrementCycleCounterGlobal(),
										  XME_STATUS_INTERNAL_ERROR);
