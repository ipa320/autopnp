 ...
 *
 * $Id: executionManager.c 5358 2013-10-01 15:58:08Z geisinger $
 */
...
xme_core_exec_configStruct_t xme_core_exec_intConfig;
...
xme_core_exec_step
(
    uint64_t maximumCycle     ///< maximum cycle number to run till; infinite if 0
);
...
)
{
    ...
    if(NULL != initStruct)
    {
        xme_core_exec_configStruct_t* initConfig =
                    (xme_core_exec_configStruct_t*) initStruct;
...
)
{
    ...
    if(xme_core_exec_dispatcher_maximumCycleReached((uint32_t)maximumCycle))
    {
...
{
    XME_LOG(XME_LOG_NOTE, MODULE_ACRONYM "finalizing execution manager\n");
