 ...
 *
 * $Id: brokerPnPManager.c 5093 2013-09-17 14:40:43Z ruiz $
 */
...
)
{
    ...
    );
    {
        if (subscriberItem->dataPacketId == dataPacketId)
        {
            ...
            XME_CHECK(
                XME_STATUS_SUCCESS == XME_HAL_SINGLYLINKEDLIST_ADD_ITEM(
                subscriberItem->subscribers,
                dataFunction), 
                XME_STATUS_INTERNAL_ERROR
            );
...
)
{
    ...
    if (xme_core_broker_isFunctionVariantRegistered(componentId, functionId, functionVariantId))
    {
        ...
        );
        {
                ...
                functionItem->functionVariantId == functionVariantId)
            {
                ...
                // fixme: maybe return value should be checked?
                (void) XME_HAL_SINGLYLINKEDLIST_REMOVE_ITEM(
                    functionItem->parameters,
    ...
    );
    {
        if (dataSubscriberItem->dataPacketId == dataPacketId)
        {
            ...
            if (XME_HAL_SINGLYLINKEDLIST_ITEM_COUNT(dataSubscriberItem->subscribers) == 0)
            {
            ...
            && functionItem->functionVariantId == functionVariantId)
        {
            if (xme_hal_singlyLinkedList_getItemCount(&(functionItem->parameters)) > 0)
            {
                    ...
                    parameterItem);
                {
                    ...
                    XME_CHECK(
                        XME_STATUS_SUCCESS == XME_HAL_SINGLYLINKEDLIST_ADD_ITEM(*dataPacketsList, &parameterItem->dataPacketId), 
                        XME_STATUS_INTERNAL_ERROR);
                }
                XME_HAL_SINGLYLINKEDLIST_ITERATE_END();
            }
        ...
        if (subscriberItem->dataPacketId == dataPacketId)
        {
            if (xme_hal_singlyLinkedList_getItemCount(&(subscriberItem->subscribers))> 0)
            {
                    ...
                    functionItem);
                {
                    // add to the data packets table output parameter
                    XME_CHECK(
                        XME_STATUS_SUCCESS == XME_HAL_SINGLYLINKEDLIST_ADD_ITEM(*functionsList, &functionItem->functionId),
                        XME_STATUS_INTERNAL_ERROR);
                }
                XME_HAL_SINGLYLINKEDLIST_ITERATE_END();
            }
