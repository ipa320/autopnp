 ...
 *
 * $Id: brokerDataManager.c 5207 2013-09-27 11:58:51Z ruiz $
 */
...
)
{
    ...
    if (!xme_core_broker_isOutputDataPacket(dataPacketId) && size > 0)
    {
    ...
    // check if the data packet is locked
    ...
    if (size > 0) // only check transfers for positive data availability
    {
        ...
        return xme_core_dataHandler_completeReadOperation(dataPacketId);
    }
    else
    {
