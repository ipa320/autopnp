 ...
 *
 * $Id: neighborhoodDetection.c 4991 2013-09-05 13:41:41Z ruiz $
 */
    ...
    XME_HAL_SINGLYLINKEDLIST_ITERATE_BEGIN(neighborList, xme_core_nodeManager_neighborhoodDetection_neighborInformation_t, currentItem);
    {
        if(--(currentItem->lastSeenBeforeTicks) <= 0)
        {
            XME_LOG(
                ...
                (void) xme_hal_singlyLinkedList_removeItem(&neighborList, currentItem, false);
                overflow = false;
                break;
        }
    }
...
(
    const xme_core_nodeManager_neighborhoodDetection_nodeAnnouncements_t* receivedPacket,
...
)
{
    ...
    XME_CHECK(NULL != receivedPacket, XME_STATUS_INVALID_PARAMETER);
    XME_CHECK(NULL != receivedInterfaceId, XME_STATUS_INVALID_PARAMETER);
    XME_CHECK(XME_CORE_NODE_INVALID_NODE_ID != receivedPacket->sendingNodeID, XME_STATUS_INVALID_PARAMETER);
    XME_CHECK(XME_COM_INTERFACE_ADDRESS_TYPE_INVALID != receivedPacket->sendingInterfaceID.addressType, XME_STATUS_INVALID_PARAMETER);
    ...
    status = xme_com_interface_genericAddressToIPv4String(&receivedPacket->sendingInterfaceID, ip, sizeof(ip));
    ...
    XME_HAL_SINGLYLINKEDLIST_ITERATE_BEGIN(neighborList, xme_core_nodeManager_neighborhoodDetection_neighborInformation_t, currentItem);
    {
        ...
        if(currentItem->senderNodeID == receivedPacket->sendingNodeID
            && currentItem->senderInterfaceID.addressType == receivedPacket->sendingInterfaceID.addressType
    ...
    if(!exists)
    {
        ...
        item->senderInterfaceID = receivedPacket->sendingInterfaceID;
        item->senderNodeID = receivedPacket->sendingNodeID;
