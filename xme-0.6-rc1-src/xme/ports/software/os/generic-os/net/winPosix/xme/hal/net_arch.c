 ...
 *
 * $Id: net_arch.c 5179 2013-09-25 16:04:59Z geisinger $
 */
...
xme_hal_net_destroySocket(xme_hal_net_socketHandle_t socketHandle)
{
    ...
    // XXX: What happens if we are not success full? then we won't remove the item from the list!
    XME_CHECK(XME_STATUS_SUCCESS == (rval = xme_hal_net_closeSocket(socketHandle)),    rval);
...
)
{
...
#if defined(_WIN32) && !defined(CYGWIN)
    {
        ...
        XME_CHECK(ioctlsocket(socketItem->socket, FIONBIO, &nonBlocking) == 0,XME_STATUS_INVALID_CONFIGURATION);
    }
#else // #if defined(_WIN32) && !defined(CYGWIN)
    XME_CHECK(fcntl(socketItem->socket, F_SETFL, blocking ? 0 : (O_NONBLOCK | FASYNC)) != -1,XME_STATUS_INVALID_CONFIGURATION);
#endif // #if defined(_WIN32) && !defined(CYGWIN)
...
)
{
    ...
    FD_ZERO(&readfds);
    //TODO: REFACTORING => internal of if could be a function because it is similar!
    if ( socketSetRead != NULL )
    {
        for ( i = 0; i < max_socket; i++ )
        {
            ...
            FD_SET(socketItem->socket, &readfds);
            if ( ((uint16_t) socketItem->socket) > maxInternalSocketHandle ) maxInternalSocketHandle = (uint16_t)socketItem->socket;
        }
    }
...
)
{
    ...
    if ( ret == 0 ) return XME_STATUS_SUCCESS;
