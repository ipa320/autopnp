...
)
{
    ...
    XME_CHECK(index < linkedListDesc->count[0], NULL);
    ...
    XME_CHECK(index != linkedListDesc->count[0] - 1U, (void*) memPool[linkedListDesc->end-1U].item);
...
)
{
    ...
    if(index >= (linkedListDesc->count[0] + 1) >> 1)
    {
        reverse = true;
        index = linkedListDesc->count[0] - index - 1;
