 ...
 *
 * $Id: linkedList_arch.c 5040 2013-09-11 10:57:37Z geisinger $
 */
 ...
 *
 * \note   This is common function used by respective singly and doubly
 *         linked lists.
 *
 ...
 *            If the callback function returns -1 for any call or 1 for all
 *            calls, insertion of the item is aborted.
 ...
 *          were NULL.
 * \retval  XME_STATUS_ABORTED if the callback function returned -1 for any
 *          call or 1 for all calls and hence insertion of the item was
 *          aborted.
...
)
{
    ...
    // If we get here, the only option is to insert at the end or to abort
    if (NULL != insertionCallback)
    {
        int insert = insertionCallback(item, NULL, userData);
...
)
{
    ...
    XME_CHECK(index < linkedList->count[0], NULL);
    ...
    XME_CHECK(index != linkedList->count[0] - 1, ((xme_hal_doublyLinkedList_genericElement_t*)linkedList->end)->item);
    ...
    if (doublyLinked && index >= (linkedList->count[0]+1) >> 1)
    {
        reverse = true;
        index = (linkedList->count[0] - index) - 1;
...
)
{
    ...
    return (void*) xme_hal_linkedList_itemFromIndex ((xme_hal_linkedList_descriptor_t *)linkedList, index, (bool) false);
}
...
{
    // TODO: Remove (void*), Issue #3388
    return (void*) xme_hal_linkedList_itemFromIndex ((xme_hal_linkedList_descriptor_t *)linkedList, index, (bool) true);
}
