 ...
 *
 * $Id: graph_arch.c 4942 2013-09-03 17:35:18Z geisinger $
 */
...
{
    xme_hal_graph_graphInternal_t* graphInternal = (xme_hal_graph_graphInternal_t*) graph;
    bool vertexIteratorWasInUse;

    XME_CHECK(graphInternal != NULL, XME_STATUS_INVALID_PARAMETER);
    vertexIteratorWasInUse = graphInternal->vertexIteratorInUse;
    ...
    // If one of the iterators has not yet been reset, only fail *after* the
    // graph has been cleared. This ensured that even in the case of improper
    // cleanup of iterators, no unnecessary memory leaks will occur.
    XME_CHECK(!vertexIteratorWasInUse, XME_STATUS_INVALID_CONFIGURATION);
