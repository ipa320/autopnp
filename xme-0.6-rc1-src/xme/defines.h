 ...
 *
 * $Id: defines.h 5163 2013-09-25 10:10:46Z camek $
 */
 ...
 *
 *         Usage examples of the XME_CHECK*() family of macros:
 *         \code
 ...
 *
 * Normally, a warning is issued during runtime if a call to an XME_ASSERT*()
 * macro contains a function call, an assignment, an increment or a decrement.
 * The reason for this is that those side effects will not be present in
 * release builds (because assertions become no-operations in release builds)
 * and hence could break the functionality. In situations where it is safe to
 * call a specific function or use one of the operators mentioned above,
 * surround the condition with the XME_ASSERT_NO_SIDE_EFFECTS() macro to
 * disable the warning.
 *
 * Typical usage scenarios of this macro include:
 * \code
 ...
 * are inspired by a StackOverflow under the wiki-cc license with
 ...
 *
 *         This macro can be used to declare a parameter that is passed to a
 ...
 *
 * \param  param The name of the parameter to declare unused.
 */
 ...
 * \param  firstArgParamIndex One-based index of the first format argument of
 *         the respective function.
 */
#ifdef __GNUC__
